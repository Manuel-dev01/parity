...
import { STABLES, getUnderlying, stableBySymbol } from "@/lib/universe";
...
/**
 * POST /api/v1/swap  { symbol, mint, usd, owner, maxDevBps, pay }
 * `pay` is the input stablecoin, symbol or mint (USDC default, USDT accepted).
...
export async function POST(req: Request) {
  let body: { symbol?: string; mint?: string; usd?: number; owner?: string; maxDevBps?: number; pay?: string };
  try {
    body = await req.json();
  } catch {
  ...
  const pay = body.pay ? (STABLES[body.pay] ? body.pay : stableBySymbol(body.pay)) : undefined;
  if (body.pay && !pay) {
    return NextResponse.json({ error: `pay must be one of ${Object.values(STABLES).map((s) => s.symbol).join(", ")} (symbol or mint)` }, { status: 400 });
  }
  let owner: string;
  try {
